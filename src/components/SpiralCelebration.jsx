...
class AnimationController {
  constructor(canvas, ctx, dpr, w, h, onComplete) {
    this.canvas = canvas;
    this.ctx = ctx;
    this.dpr = dpr;
    this.w = w;
  ...
  setupTimeline() {
    this.timeline = gsap.timeline({ repeat: 0, onComplete: () => this.handleComplete() });
  ...
  showProjectedDot(position, sizeFactor, color) {
...
export default function SpiralCelebration({ show, onComplete }) {
  ...
  useEffect(() => {
    ...
    animationRef.current = new AnimationController(canvas, ctx, dpr, w, h, () => {
      setTimeout(() => {
        setVisible(false);
  ...
  return (
    <div
      className={`spiral-celebration ${visible ? "visible" : ""}`}
      style={{
        position: "fixed",
        inset: 0,
        zIndex: 9999,
